...
create_status_cell <- function(status, completeness = NULL) {
  ...
  if(percentage == 100) {
    ...
    return('<div style="width: 20px; height: 20px; background: #705555; border: 1px solid #983248; margin: 0 auto;"></div>')
...
for(site_id in site_order) {
  if(site_id %in% names(site_status_list)) {
    ...
    site_name <- gsub("University", "Uni.", site_data$site_name)
...
create_column_headers <- function() {
  ...
  for(site_id in site_order) {
    if(site_id %in% names(site_status_list)) {
      ...
      cylinder_color <- if(completion == 100) '#7d1538' else if(completion >= 75) '#705555' else if(completion >= 50) '#e6a1a1' else '#9c8989'
...
mini_cylinders_html <- paste0(
  ...
  paste(sapply(site_order, function(site_id) {
    if(site_id %in% names(site_status_list)) {
      ...
      # Determine cylinder color
      cylinder_color <- if(completion == 100) '#7d1538' else if(completion >= 75) '#983248' else if(completion >= 50) '#c4898d' else '#9c8989'
...
legend_html <- '<div style="margin-top:0.25rem; background:#f8f9fa; border:1px solid #dee2e6; border-radius:4px; padding:0.25rem 0.5rem;">
  ...
  <div style="display:flex; flex-direction:row; gap:1rem; justify-content:center; align-items:center; flex-wrap:wrap;">
    ...
    <div style="display:flex; align-items:center; font-size:1rem;">
      <span style="display:inline-block; width:18px; height:18px; background:#705555; border:1.5px solid #983248; margin-right:8px;"></span>
      <strong>Partial</strong> <span style="color:#666; margin-left:5px;">(All required columns and some mCIDE)</span>
    </div>
        <div style="display:flex; align-items:center; font-size:1rem;">
      <span style="display:inline-block; width:18px; height:18px; background:#e6a1a1; border:1.5px solid #5d1028; margin-right:8px;"></span>
      <strong>In Progress</strong> <span style="color:#666; margin-left:5px;"></span>
    </div>
    <div style="display:flex; align-items:center; font-size:1rem;">
      ...
      <strong>Not Started</strong> <span style="color:#666; margin-left:6px;"></span>
    </div>
  </div>
</div>'
