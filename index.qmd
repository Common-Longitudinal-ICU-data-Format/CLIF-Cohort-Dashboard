...
# table_status <- read.csv("data/table_status.csv", stringsAsFactors = FALSE)  # Now using site_status_matrix.csv
# status_summary <- jsonlite::fromJSON("data/table_status_summary.json")  # Now using site_status_matrix.csv

# Read site-specific status data
# site_status_aggregated <- jsonlite::fromJSON("data/site_status_aggregated.json")  # Now using CSV files
...
create_status_cell <- function(status, completeness = NULL) {
  ...
  # Use exact status matching instead of percentage logic to match our 3-color legend
  if(status_lower == "complete") {
    # Complete - Dark maroon (matches legend)
    return('<div style="width: 20px; height: 20px; background: #7d1538; border: 1px solid #5d1028; margin: 0 auto;"></div>')
  } else if(status_lower == "partial") {
    # Partial - Light pink (matches legend)
    return('<div style="width: 20px; height: 20px; background: #e6a1a1; border: 1px solid #5d1028; margin: 0 auto;"></div>')
  } else {
    # Incomplete - Gray (matches legend)
    return('<div style="width: 20px; height: 20px; background: #9c8989; border: 1px solid #5d1028; margin: 0 auto;"></div>')
  }
}

# Helper function to create site status summary
create_site_status_summary <- function() {
  # Read the site status matrix CSV
  site_status_matrix <- read_csv("data/site_status_matrix.csv", show_col_types = FALSE)

  # Extract site names from the CSV (excluding Table_Name column)
  site_names <- colnames(site_status_matrix)[-1]

  site_summary <- data.frame()

  # Calculate completion for each site
  for(site_name in site_names) {
    site_col <- site_status_matrix[[site_name]]
    statuses <- tolower(site_col)

    complete_count <- sum(statuses == "complete")
    partial_count <- sum(statuses == "partial")
    total_count <- length(statuses)

    # Calculate weighted completion (complete=100%, partial=75%, incomplete=0%)
    completion <- round((complete_count * 100 + partial_count * 75) / total_count)

    site_summary <- rbind(site_summary, data.frame(
      site_name = site_name,
      completion = completion,
      complete_tables = complete_count,
      total_tables = total_count,
      status = if(completion >= 90) "Ready" else "Pending",
...
master_cylinder_html <- paste0(
  ...
  '<div style="font-size: 0.75rem; color: #999; margin-top: 1rem;">Last Updated: ', format(Sys.Date(), "%B %d, %Y"), '</div>',
  '</div>',
...
```{=html}
<style>
/* Scope styles to Status Tracker tab only */
...
.status-tracker-container .bslib-card {
  height: calc(100vh - 120px) !important;
  min-height: calc(100vh - 120px) !important;
}

.status-tracker-container .card-body {
...
.status-tracker-container #consolidated-status-matrix_wrapper {
  height: 100% !important;
  min-height: 100% !important;
  flex: 1 !important;
}

.status-tracker-container #consolidated-status-matrix {
  height: 100% !important;
}

.status-tracker-container .dataTables_scrollBody {
  height: auto !important;
  min-height: auto !important;
}

.status-tracker-container .dataTables_info,
.status-tracker-container .dataTables_paginate,
.status-tracker-container .dataTables_filter,
.status-tracker-container .dataTables_length,
.status-tracker-container input[type="search"],
.status-tracker-container .form-control[type="search"] {
...
# Create matrix data starting with table names
matrix_data <- data.frame(
  `Table Name` = site_status_matrix$Table_Name,
  stringsAsFactors = FALSE
)

# Add columns for each site with status cells
for (site_name in site_names) {
  site_column <- character(nrow(site_status_matrix))

  # Add status for each table using existing create_status_cell function
  for(i in seq_along(site_status_matrix[[site_name]])) {
    status <- site_status_matrix[[site_name]][i]
    completeness_score <- case_when(
      tolower(status) == "complete" ~ 1.0,
      tolower(status) == "partial" ~ 0.75,
      TRUE ~ 0.5
    )
    site_column[i] <- create_status_cell(status, completeness_score)
  }

  matrix_data[[site_name]] <- site_column
}

# Calculate completion percentages for each site
calculate_site_completion <- function(site_col) {
  statuses <- tolower(site_col)
  complete_count <- sum(statuses == "complete")
  partial_count <- sum(statuses == "partial")
  total_count <- length(statuses)
...
mini_cylinders_html <- paste0(
  '<div style="margin-bottom: 0; padding:0rem;">',
  ...
  # Empty cell for table name column - INCREASED WIDTH to 240px
  '<td style="width: 240px; padding: 2.75rem; text-align: left; font-size: 0.8rem; font-weight: 600; color: #495057; border-right: 1px solid #dee2e6; box-sizing: border-box;">Site Progress</td>',
  
  # Mini cylinders for each site
  paste(sapply(site_names, function(site_name) {
    # Get site completion stats
    site_stats <- calculate_site_completion(site_status_matrix[[site_name]])
    completion <- site_stats$completion
    complete_tables <- site_stats$complete_tables
    total_tables <- site_stats$total_tables

    # Determine cylinder color based on completion
    cylinder_color <- if(completion >= 90) '#7d1538' else if(completion >= 70) '#983248' else if(completion >= 50) '#e6a1a1' else '#9c8989'
      ...
      paste0(
        ...
        # Cylinder body
        '<div style="position: absolute; bottom: 0; left: 2px; width: 26px; height: 35px; background: linear-gradient(90deg, #f8f9fa 0%, #ffffff 50%, #f1f3f4 100%); border: 2px solid #7d1538; border-radius: 0 0 4px 4px; box-shadow: inset 1px 0 2px rgba(0,0,0,0.1);"></div>',
        ...
        # Top
        '<div style="position: absolute; top: 0; left: 2px; width: 26px; height: 10px; background: linear-gradient(90deg, #e9ecef 0%, #ffffff 50%, #e9ecef 100%); border: 2px solid #7d1538; border-radius: 50%;"></div>',
        
        '</div>',
        
        # Site name and stats below cylinder
...
legend_html <- '<div style="margin-top:0.25rem; background:#f8f9fa; border:1px solid #dee2e6; border-radius:4px; padding:0.25rem 0.5rem;">
  <h6 style="margin:0 0 0.25rem 0; color:#495057; font-size:0.95rem; font-weight:700; text-align:center;">Status Legend</h6>
  <div style="display:flex; flex-direction:row; gap:1rem; justify-content:center; align-items:center; flex-wrap:wrap;">
    <div style="display:flex; align-items:center; font-size:1rem;">
      ...
      <strong>Complete</strong> <span style="color:#666; margin-left:5px;"></span>
    </div>
    <div style="display:flex; align-items:center; font-size:1rem;">
      <span style="display:inline-block; width:18px; height:18px; background:#e6a1a1; border:1.5px solid #5d1028; margin-right:8px;"></span>
      <strong>Partial</strong> <span style="color:#666; margin-left:5px;"></span>
    </div>
    <div style="display:flex; align-items:center; font-size:1rem;">
      <span style="display:inline-block; width:22px; height:22px; background:#9c8989; border:1.5px solid #5d1028; margin-right:10px;"></span>
      <strong>Incomplete</strong> <span style="color:#666; margin-left:6px;"></span>
    </div>
  </div>
</div>'
